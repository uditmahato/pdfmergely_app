...
### Windows build fixes (apply after every fresh `expo prebuild`)

`android/` is gitignored and regenerated by prebuild, so these two local tweaks must be
re-applied whenever it is regenerated:

1. **CMake 3.31 override** — the SDK's default CMake 3.22.1 hits the Windows ninja
   "manifest 'build.ninja' still dirty after 100 tries" loop when compiling
   react-native-reanimated / expo-modules-core. Install once via
   `sdkmanager "cmake;3.31.0"`, then add to `android/local.properties`:

   ```properties
   cmake.dir=C:/Users/<you>/AppData/Local/Android/Sdk/cmake/3.31.0
   ```

2. **ABI filter** — in `android/gradle.properties`, set
   `reactNativeArchitectures=x86_64,arm64-v8a` (emulator + modern phones). Halves
   native build time; add ABIs back only if you need to ship to very old devices.

Known gotcha: do not use `<Link asChild>` around a `Pressable` that has a
function-form `style` — expo-router's prop-cloning silently drops the style.
Use `useRouter().push(...)` in `onPress` instead (see `src/app/index.tsx`).
